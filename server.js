...
app.delete('/quiz/:id', (req, res) => {
  const store = loadStore(); delete store[req.params.id]; saveStore(store);
  res.json({ ok: true });
});
...
app.get('/quiz/:id', (req, res) => {
  ...
  // Helper: set cookie (NOT httpOnly so JS can read for localStorage backup)
  const setCookie = (token) => res.cookie(cookieName, token, {
  ...
  // Already claimed — check cookie
  if (sessionCookie === quiz.sessionToken) return res.send(quizPage(req.params.id, quiz));

  // Cookie missing — check ?t= recovery token from localStorage redirect
  if (queryToken && queryToken === quiz.sessionToken) {
    setCookie(quiz.sessionToken);
    return res.send(quizPage(req.params.id, quiz));
  }

  // Truly different user — show claimed page with localStorage recovery script
...
function claimedPage(quizId) {
  // Includes recovery script: if this browser has the token in localStorage, auto-redirect
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Reconnecting…</title>
  <link href="https://fonts.googleapis.com/css2?family=Sora:wght@400;600&display=swap" rel="stylesheet"/>
  <style>body{background:#090b18;color:#fff;font-family:'Sora',sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;padding:24px;}
  .box{text-align:center;max-width:400px;}h1{font-size:2rem;color:#f59e0b;margin-bottom:12px;}p{color:#64748b;line-height:1.6;font-size:.9rem;}
  ...
  <p id="msg" style="display:none;color:#ef4444;">This quiz is already in use by another session.</p>
  </div>
  <script>
    var tok = localStorage.getItem('qt_${quizId}');
    if (tok) { location.href = '/quiz/${quizId}?t=' + encodeURIComponent(tok); }
    else { document.getElementById('spinner').style.display='none'; document.getElementById('msg').style.display='block'; document.querySelector('p').textContent=''; }
  </script></body></html>`;
}
...
<head>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    .qnav-dot.d-current{background:rgba(59,130,246,.6);}
    .qnav-dot.d-correct{background:rgba(34,197,94,.6);}
    .qnav-dot.d-wrong{background:rgba(239,68,68,.6);}
    ...
    @media(max-width:520px){
      ...
      .btn{font-size:.85rem;padding:13px;} .btn-prev{min-width:80px;}
...
// ── localStorage session backup (cookie recovery) ─────────────────────────────
...
let corrections        = {};
let results            = [];  // { number, correct, selectedLetter } — no skips stored here until submit
...
function selectOption(letter, btn) {
  ...
  document.getElementById('changeAnswerWrap').className = 'change-answer-wrap show';
  const sentCorr = corrections[q.number]?.sent;
  if (!correct && !sentCorr) document.getElementById('disagreeWrap').className = 'disagree-wrap show';
  const nextBtn = document.getElementById('nextBtn');
  nextBtn.classList.remove('skip-mode');
  nextBtn.textContent = isLast ? 'Submit & Finish 🎯' : 'Next →';
...
function showCorrectionPanel() {
  ...
  getOptions(q).forEach(o => {
    ...
    btn.onclick = () => {
      ...
      btn.classList.add('selected'); selectedCorrection = o.letter;
    };
    container.appendChild(btn);
  });
}
...
function sendCorrection() {
  ...
  corrections[q.number] = {
    ...
    user_correction_letter: selectedCorrection, user_correction_text: corrText,
    explanation: expl, sent: true
  };
  saveState();
...
function nextQuestion() {
  const isLast = current === QUESTIONS.length - 1;
  if (isLast) {
    // Count unanswered (no result entry at all)
...
async function doSubmit() {
  ...
  // Fill in skipped questions as wrong
  QUESTIONS.forEach(q => {
...
function renderQNav() {
  ...
  grid.innerHTML = QUESTIONS.map((q, i) => {
    ...
    // skipped = no class added (stays grey/unanswered)
...
(function initBackground() {
  ...
  const N      = mobile ? 40 : 75;

  // Multi-color palette: blue, purple, teal, amber, pink
  const PALETTE = [
    [96, 165, 250],   // blue
    [168, 85, 247],   // purple
    [45, 212, 191],   // teal
  ...
  function draw() {
    ...
    for (let i = 0; i < dots.length; i++) {
      for (let j = i + 1; j < dots.length; j++) {
        ...
        if (d < 120) {
          const c = dots[i].col;
          ctx.beginPath();
          ctx.strokeStyle = 'rgba(' + c[0] + ',' + c[1] + ',' + c[2] + ',' + (0.1 * (1 - d / 120)) + ')';
          ctx.lineWidth   = 0.7;
    ...
    dots.forEach(d => {
      ...
      ctx.fillStyle = 'rgba(' + d.col[0] + ',' + d.col[1] + ',' + d.col[2] + ',' + d.op + ')';
      ctx.fill();
    });
